			...
			function captureDataCurlAttr (tag, attrs) {
				...
				if (!src) {
					...
					if (isCurl) {
						...
						output = run.length > 1 ? run.shift() : src;
						input = run[0];
					...
					if (output == src) output += '.cram';
					io.info('Bundling ' + input + ' into ' + output);
					cfg = grokConfig(input).then(function (cfg) {
						// TODO: this could fail if there are more than one configs in run.js
						cfg[0].config.baseUrl = makePath(cfg[0].config.baseUrl);
						cfg[0].output = output;
						return cfg;
					});
