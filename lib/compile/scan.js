...
define([], function () {
	...
	findDefinesRx = new RegExp(
		...
		+ '|(define)\\s*\\(' // define call
		+ '|(?:["\']([^"\']*)["\']\\s*,)' // id (with comma)
		+ '|(?:\\[([^\\]]*?)\\]\\s*,)' // deps array (with comma)
		// find "function id? (args?) {" OR "{"
		// Note: \\s*[^(\\s]* fails on multiple words to try to catch "function"
		// in a comment.  This is problematic: we should do two passes.
		// TODO: do two passes. First pass can replace comments with spaces.
		+ '|(?:(function)\\s*[^(\\s]*\\s*\\(([^)]*)\\))?\\s*({)'
		// block comments
		+ '|(\\/\\*)|(\\*\\/)'
		// line comments
		...
		// escaped quotes are captured, but ignored below
		+ '|(\\\\["\'])|(["\'])'
	...
	function scan (source) {
		...
		function amdParser (m, rval, def, id, deps, factory, args, fStart, bcStart, bcEnd, lcStart, lcEnd, escQ, q, pStart, pEnd, matchPos) {

			// optimization: just ignore escaped quotes
			if (escQ) return '';
