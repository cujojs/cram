...
function (when, sequence, getCtx, scan) {
	...
	function inExcludeList(parentCtx, moduleStr) {
		var inList = false;
		(parentCtx.config.excludes || []).forEach(function(ignoreModule) {
			if (moduleStr == ignoreModule) inList = true;
	...
	function parseModules (io, parentCtx, fileCtx) {
		...
		// scan for AMD meta data, and remove ignored modules
		results.filter(function(res) {
			if (res.depList) {
				res.depList = res.depList.filter(function(dep) {
					return !inExcludeList(parentCtx, dep);
				});
			}
			if (res.requires) {
				res.requires = res.requires.filter(function(req) {
					return !inExcludeList(parentCtx, req.id);
				});
			}
		});
